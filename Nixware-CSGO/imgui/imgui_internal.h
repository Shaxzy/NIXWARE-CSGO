...
namespace ImGui
{
	...
	IMGUI_API bool          MyButtonEx(const char* label, const char* icon, ImFont* icon_font, bool inuse, const ImVec2& size_arg, ImGuiButtonFlags flags);
	...
	IMGUI_API bool          SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, const char* display_format, ImGuiSliderFlags flags = 0);
	...
	IMGUI_API bool          InputScalarAsWidgetReplacementCustom(const ImRect& aabb, const char* label, ImGuiDataType data_type, void* data_ptr, ImGuiID id, int decimal_precision);
