...
bool ImGui::BeginPopupModal(const char* name, bool* p_open, ImGuiWindowFlags extra_flags)
{
	...
	ImGuiWindowFlags flags = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_Popup | ImGuiWindowFlags_Modal;
	bool is_open = BeginAndroid(name, p_open, false, flags);
	ImGui::GetWindowDrawList()->AddRectFilledMultiColor(ImVec2(ImGui::GetWindowPos().x + GetWindowWidth() / 2, ImGui::GetWindowPos().y - 6), ImVec2(ImGui::GetWindowPos().x + ImGui::GetWindowSize().x, ImGui::GetWindowPos().y + 2), Vars.barColor1.x, Vars.barColor1.y, Vars.barColor1.h, Vars.barColor1.w);
...
bool ImGui::BeginAndroid(const char* name, bool* p_open, bool mainframe, ImGuiWindowFlags flags)
{
	ImGuiContext& g = *GImGui;
	const ImGuiStyle& style = g.Style;
	IM_ASSERT(name != NULL);                        // Window name required
	...
	if (first_begin_of_the_frame)
	{
		...
		else
		{
			...
			if (mainframe && Vars.days <= 5) {
				ImGui::SetCursorPos(ImVec2(530, -11));
				Image(CMenu::Get().anime_picture, ImVec2(278, 393));
			}
			// // TODO : LINES
			if (mainframe) {
				window->DrawList->AddRectFilled(window->Pos + ImVec2(132, 0), window->Pos + window->Size, GetColorU32(ImGuiCol_Border), window_rounding);
				window->DrawList->AddRectFilled(window->Pos + ImVec2(132, 0), window->Pos + ImVec2(133, 244), GetColorU32(ImVec4(1.00f, 0.0f, 0.0f, 1.00f)), window_rounding);
				window->DrawList->AddRectFilled(window->Pos + ImVec2(132, 240), window->Pos + ImVec2(133, window->Size.y - 35), GetColorU32(ImVec4(1.00f, 0.0f, 0.0f, 1.00f)), window_rounding);
			}
			...
			if (mainframe) {
				window->DrawList->AddRectFilled(window->Pos - ImVec2(0, -347), window->Pos + ImVec2(window->Size.x - 2, window->Size.y + 1), ImColor(0.15625f, 0.15625f, 0.15625f, 1.00f), window_rounding);
				window->DrawList->AddRectFilledMultiColor(ImVec2(window->Pos.x + 4, window->Pos.y + 344), ImVec2(window->Pos.x + window->Size.x - 4, window->Pos.y + 346), ImColor(255, 0, 0), ImColor(255, 0, 0), ImColor(255, 0, 0), ImColor(255, 0, 0));
				window->DrawList->AddRect(window->Pos - ImVec2(-3, 0), window->Pos + ImVec2(window->Size.x - 3, window->Size.y + -34), ImColor(0.1f, 0.1f, 0.1f, 1.00f));
			}
			//window->DrawList->AddRect(ImGui::GetWindowPos() - ImVec2(1, 5), window->Pos + ImVec2(window->Size.x + 1, window->Size.y + -31), ImColor(0.1f, 0.1f, 0.1f, 1.00f));
...
bool ImGui::ButtonGradientEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	// Render
	const ImU32 col = GetColorU32((hovered && held) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
	window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, ImColor(40, 40, 40), ImColor(40, 40, 40), ImColor(0, 0, 0), ImColor(0, 0, 0));
...
// ImFont//
bool ImGui::MyButtonEx(const char* label, const char* icon, ImFont* icon_font, bool inuse, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	if (inuse)
	{
		...
		// lines
		RenderFrame(ImVec2(bb.Min.x - 25, bb.Min.y + 1), ImVec2(bb.Max.x, bb.Min.y), GetColorU32(ImVec4(1.00f, 0.0f, 0.0f, 1.00f))); // ImVec4(1.07f, 1.07f, 1.07f, 1.00f)
		RenderFrame(ImVec2(bb.Min.x - 25, bb.Max.y - 1), ImVec2(bb.Max.x, bb.Max.y), GetColorU32(ImVec4(1.00f, 0.0f, 0.0f, 1.00f)));
	}
	else if (!inuse)
		RenderFrame(ImVec2(bb.Max.x - 1, bb.Min.y - 4), ImVec2(bb.Max.x, bb.Max.y + 4), GetColorU32(ImVec4(1.00f, 0.0f, 0.0f, 1.00f)));
	// ICON TEXT
	if (icon)
	{
		// icon
		...
		RenderText(pos + ImVec2(10, size_arg.y / 2 - icon_size.y / 2), icon);
		PopFont();
	}

	RenderText(bb.Min + ImVec2(45, size_arg.y / 2 - label_size.y / 2), label);

	return pressed;
}

bool ImGui::MyButton(const char* label, const char* icon, ImFont* icon_font, bool inuse, const ImVec2& size_arg)
{
	return MyButtonEx(label, icon, icon_font, inuse, size_arg, 0);
}
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, const char* display_format, ImGuiSliderFlags flags)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	const ImGuiStyle& style = g.Style;
	...
	window->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(0, 3), grab_bb.Max - ImVec2(0, 3), ImColor(255, 0, 0), ImColor(255, 0, 0), ImColor(100, 0, 0), ImColor(100, 0, 0));
	...
	float textsizex = value_bufsize.x;
	float posx = grab_bb.Max.x - textsizex;
	if (posx < frame_bb.Min.x) posx = frame_bb.Min.x;

	RenderText(ImVec2(posx /*/2*/, grab_bb.Max.y - 4), value_buf, value_buf_end, NULL);
	return value_changed;
}
...
bool ImGui::InputScalarAsWidgetReplacementCustom(const ImRect& aabb, const char* label, ImGuiDataType data_type, void* data_ptr, ImGuiID id, int decimal_precision)
{
	Spacing();
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
	...
	if (start_text_input || (g.ActiveId == id && g.ScalarAsInputTextId == id)) {
		return InputScalarAsWidgetReplacementCustom(frame_bb, label, ImGuiDataType_Float, v, id, decimal_precision);
	}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(33, 14));
	...
	if (*v)
	{
		...
		window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(1, 1) - ImVec2(20, 1), ImColor(255, 0, 0), ImColor(255, 0, 0), ImColor(100, 0, 0), ImColor(100, 0, 0));
	}
	else
	{
		window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max - ImVec2(20, 1), ImColor(0.1f, 0.1f, 0.1f));
		window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(1, 1) - ImVec2(20, 1), ImColor(49, 51, 47), ImColor(49, 51, 47), ImColor(32, 33, 31), ImColor(32, 33, 31));
	}

	if (label_size.x > 0.0f)
		RenderText(text_bb.GetTL() - ImVec2(16, 4), label);
...
bool ImGui::Combo(const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
	...
	static float arg; arg = !strcmp("##configselect", label) ? 4.5f : 0.f;

	if (!BeginCombo(label, preview_text, arg, 0))
		return false;
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	if (/*hovered || */selected)
	{
		...
		window->DrawList->AddRectFilledMultiColor(bb_with_spacing.Min, bb_with_spacing.Max, ImColor(255, 0, 0), ImColor(255, 0, 0), ImColor(100, 0, 0), ImColor(100, 0, 0));
