...
void CMenu::RenderLegitbotTab()
{
	...
	case 1:
		...
		else
		{
			...
			if (Globals::LocalPlayer->m_hActiveWeapon()->IsRifle()) {
...
void CMenu::RenderRagebotTab()
{
	...
	if (ImGui::ButtonSelectable(XorStr("Aimbot"), ImVec2(220, 40), selected == 0, forTabs_text, forTabs_text_x2))
		selected = 0;
	ImGui::SameLine();
	if (ImGui::ButtonSelectable(XorStr("Helpers"), ImVec2(220, 40), selected == 1, forTabs_text, forTabs_text_x2))
		selected = 1;
	...
	if (selected == 0) {
		...
		ImGui::Checkbox(XorStr("Auto Scope"), &Vars.ragebot_autoscope);
		ImGui::Checkbox(XorStr("Auto Stop"), &Vars.ragebot_autostop);
		ImGui::Checkbox(XorStr("Auto Crouch"), &Vars.ragebot_autocrouch);

		ImGui::SliderInt(XorStr("FOV"), &Vars.ragebot_fov, 0, 180);
		ImGui::SliderFloat(XorStr("Hitchance"), &Vars.ragebot_hitchance, 0.f, 100.f);
		ImGui::SliderFloat(XorStr("Min Damage"), &Vars.ragebot_mindamage, 0.f, 100.f);
		ImGui::SliderInt(XorStr("Baim after x shots"), &Vars.ragebot_baim_after_shots, 0, 10);
		ImGui::Checkbox(XorStr("Auto baim"), &Vars.ragebot_autobaim);
		ImGui::Checkbox(XorStr("Auto zeus"), &Vars.ragebot_autozeus);

		/*ImGui::Checkbox(XorStr("Backtracking"), &Vars.ragebot_backtrack);
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip(XorStr("Can be crashes"));*/

		ImGui::Combo("Target Selection", &Vars.ragebot_selection, selectiontypes, IM_ARRAYSIZE(selectiontypes));

		static std::string prevValue = "";
		if (ImGui::BeginCombo("Hitscan", prevValue.c_str(), 0))
		{
			prevValue = "";
			std::vector<std::string> vec;
			for (size_t i = 0; i < IM_ARRAYSIZE(hitboxes); i++)
			{
				ImGui::Selectable(hitboxes[i], &Vars.ragebot_hitbox[i], ImGuiSelectableFlags_::ImGuiSelectableFlags_DontClosePopups);
		...
		for (int i = 0; i < 8; i++) {
			if (Vars.ragebot_hitbox[i])
				ImGui::SliderFloat(hitboxes[i], &Vars.ragebot_hitbox_multipoint_scale[i], 0.f, 1.f);
		}
		...
		ImGui::SliderInt("Slowwalk (Shift Key)", &Vars.ragebot_slowwalk_amt, 0, 100);
...
void CMenu::RenderVisualsTab()
{
	...
	case 1:
		...
		ImGui::Checkbox(XorStr("Thirdperson (V Key)"), &Vars.visuals_thirdperson);
