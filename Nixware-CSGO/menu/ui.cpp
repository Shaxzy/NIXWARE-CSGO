...
//template <class T>
//bool ComboForSkins(const char* label, std::vector current_item, bool(*items_getter)(PVOID data, int idx, const char** out_text), PVOID data, int items_count, int popup_max_height_in_items = -1) {
//	
//}
...
static bool Items_ArrayGetter(PVOID data, int idx, const char** out_text)
{
...
static auto vector_getter = [](PVOID vec, int idx, const char** out_text) {
...
bool ImGui::Combo(const char* label, int* currIndex, std::vector<std::string>& values) {
	...
	return ImGui::Combo(label, currIndex, vector_getter,
		static_cast<PVOID>(&values), values.size());
}
...
bool ImGui::ListBox(const char* label, int* current_item, std::string items[], int items_count, int height_items) {
	...
	const bool value_changed = ImGui::ListBox(label, current_item, Items_ArrayGetter, static_cast<PVOID>(tmp), items_count, height_items);
...
{
	return ImGui::Combo(label, current_item, [](PVOID data, int idx, const char** out_text)
	{
		*out_text = (*reinterpret_cast<std::function<const char*(int)>*>(data))(idx);
		return true;
	}, &lambda, items_count, height_in_items);
