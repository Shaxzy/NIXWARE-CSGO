...
public:
    virtual bool RegisterConCommandBase(ConCommandBase *pVar)
    {
        ...
        Interfaces::Convar->RegisterConCommand(pVar);
        return true;
    }
};
...
{
    if(!Interfaces::Convar || s_bRegistered)
        return;

    assert(s_nDLLIdentifier < 0);
    s_bRegistered = true;
    s_nCVarFlag = nCVarFlag;
    s_nDLLIdentifier = Interfaces::Convar->AllocateDLLIdentifier();
...
void ConVar_Unregister()
{
    ...
    assert(s_nDLLIdentifier >= 0);
    Interfaces::Convar->UnregisterConCommands(s_nDLLIdentifier);
    s_nDLLIdentifier = -1;
    s_bRegistered = false;
}
...
void ConCommandBase::Shutdown()
{
    if(Interfaces::Convar) {
        Interfaces::Convar->UnregisterConCommand(this);
    }
}
...
void ConVar::ChangeStringValue(const char *tempVal, float flOldValue)
{
    ...
    if(Interfaces::Convar)
        Interfaces::Convar->CallGlobalChangeCallbacks(this, pszOldValue, flOldValue);
}
