...
{
	return BASS_MusicLoad(mem, (const PVOID)file, offset, length, flags | BASS_UNICODE, freq);
}

static inline HSAMPLE BASS_SampleLoad(BOOL mem, const WCHAR *file, QWORD offset, DWORD length, DWORD max, DWORD flags)
{
	return BASS_SampleLoad(mem, (const PVOID)file, offset, length, max, flags | BASS_UNICODE);
}

static inline HSTREAM BASS_StreamCreateFile(BOOL mem, const WCHAR *file, QWORD offset, QWORD length, DWORD flags)
{
	return BASS_StreamCreateFile(mem, (const PVOID)file, offset, length, flags | BASS_UNICODE);
}
...
{
	return BASS_SetConfigPtr(option | BASS_UNICODE, (const PVOID)value);
}
#endif
#endif
